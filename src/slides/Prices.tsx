...
const Prices = () => {
  ...
  const handleOnMouseMove = (e:React.MouseEvent) => {
    if(activeSlide.current) {
      actuallyPosition.current = positionBeforeMove.current - (startPosition.current - e.clientX);
      if(actuallyPosition.current < (-350*priceBoxes.length)-1150) {
        actuallyPosition.current = (-350*priceBoxes.length)-1150;
  ...
  return (
    ...
    >
      <div
        style={{minWidth: `${350*priceBoxes.length}px`, cursor: 'pointer'}}
        ref={trackSlider}
      >
          ...
          <div key={index} className={style.variant}>
            ...
            <div className={style.title}>
              ...
              <span className={style.price}>
                ...
                <span>.{String(box.cost[1]).length < 2 ? String(box.cost[1])+'0' : box.cost[1]} zł</span>
              </span>
            </div>
            {box.cons.map(con => (
              <div className={style.addon + ' ' + (con.have ? '' : style.minus)}>
