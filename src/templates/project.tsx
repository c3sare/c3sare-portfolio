  ...
  return (
    <Layout>
      ...
      <div className={style.slider} ref={slideShow}>
        <div className={style.sliderContainer} style={{width: page.images.length*currentWidth+"px", transform: `translate3d(${-currentWidth*currentSlide}px, 0px, 0px)`}}>
          {page.images.map((screen:any) => (
            <GatsbyImage style={{width: currentWidth+"px"}} image={getImage(screen)} alt={screen.title}/>
          ))}
        </div>
        {page.images.length > 1 &&
          <>
            <div className={style.slideBtnLeft} onClick={handlePrevSlide}><FaArrowCircleLeft/></div>
            <div className={style.slideBtnRight} onClick={handleNextSlide}><FaArrowCircleRight/></div>
          </>
        }
      </div>
      <div className={style.techUsed}>
        ...
        <div className={style.techs}>
          {page.technologies.map((tech:any) => (
            <div className={style.tech}>
              <img src={tech.img.url} alt={tech.name} width="32px" height="auto"/>
              <span>{tech.name}</span>
            </div>
          ))}
        </div>
        <div className={style.siteUrl}>
          <h3>Adres Strony</h3>
          <a target="_blank" href={page.demo}>{page.demo}</a>
        </div>
      </div>
